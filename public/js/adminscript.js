var MASTERPASS = '', OpenSpice = (function() {
    ...
    p.templates = {
        ...
        trackInQueue: _.template('<tr><td><i class="icon-music"></i></td><td><%= name %></td><td><%= artists %></td><td><%= album%></td><td><button class="btn fnct_plus <%= disabled%>"><i class="icon-plus"></i></button></td></tr>')
    };
...
var passcheck = function(p, i){
    ...
    $.ajax({
            ...
            if(JSON.parse(data).valid){
                ...
                console.log(ii);
                if(ii < 5){
                    passcheck(prompt('admin password ?'), ii);
                }else{
                    alert('FAIL!');
                }
            }
...
$(function() {
    ...
    passcheck(prompt('admin password ?'));
